...
#### コンテナ接続
特定のワークロードをクリックした後、**実行中のコンテナ**タブで**コンテナ接続**をクリックしてコンテナに接続できます。
コンテナに接続するためにはイメージにbashやshのようなshellがインストールされている必要があります。

> [参考]
> コンテナ接続後、3分間動作がない場合、接続が終了します。
> コンテナのlocale設定により、他の言語は入力できない場合があります。
