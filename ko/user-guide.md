...
| 내부 요청 응답 대기 시간 | 다른 워크로드의 내부 로드 밸런서의 vip로 통신을 요청할때 적용되는 타임아웃입니다.<br>설정된 시간내에 Connection이 사용되지 않는 경우 Connection은 삭제됩니다.<br>Connection을 계속 유지하려면 설정된 시간내에 메시지가 전송되어야 합니다.(예. keepalive 설정) |
